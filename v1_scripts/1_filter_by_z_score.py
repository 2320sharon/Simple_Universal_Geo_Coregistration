...
def process_and_plot_outliers(df: pd.DataFrame, z_threshold: float = 2, combined_z_plot_filename: str = 'combined_z_scores.png', shifts_plot_filename: str = 'plot_outlier_shifts.png') -> pd.DataFrame:
    """
...
def filter_shifts(df: pd.DataFrame, min_shift_meters: tuple, max_shift_meters: tuple) -> pd.DataFrame:
    """
...
# Variables
min_shift_meters = (-200,-200) # for S2 where each pixel is 10m this is 20 pixels
max_shift_meters = (200,200)
z_threshold=1.5

result_json_path = r"C:\development\doodleverse\coastseg\CoastSeg\data\ID_1_datetime11-04-24__04_30_52\coregistered\S2\ms\transformation_results.json"
with open(result_json_path, 'r') as json_file:
    results = json.load(json_file)

# turn this into a pandas dataframe
df = pd.DataFrame(results)
# make the filename into the row index
df = df.transpose()

# replace nan values with 0 ( otherwise the z-score calculation will fail)
df.fillna(0.0, inplace=True)

# identify and plot outliers
df = process_and_plot_outliers(df, z_threshold=z_threshold, combined_z_plot_filename='combined_z_scores.png', shifts_plot_filename='plot_outlier_shifts.png')
# filter shifts based on ranges
df = filter_shifts(df, min_shift_meters, max_shift_meters)

# make the filename into a column
df['filename'] = df.index

# drop the index
df.reset_index(drop=True, inplace=True)
# make the filename the first column
